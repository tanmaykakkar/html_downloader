def _mkdir_if_not_exists(directory):
    ...
    if not os.path.exists(directory):
        logging.debug("Making dir {0}".format(directory))
...
def _write_html_to_file(filename, content):
    ...
    with open(filename, constants.FILE_OPEN_MODE_WRITE_PLUS) as f:
...
def write_to_filesystem(html_obj):
    ...
    # i.e.- http://facebook.com/a/b/c
        ...
        html_file_name = ''.join([url_path_componets.pop(), constants.FILE_NAME_SUFFIX])
        html_files_endpoint_rec_dir = os.path.join(html_files_hostname_root, '/'.join(url_path_componets))
        _mkdir_if_not_exists(html_files_endpoint_rec_dir)
        abs_html_file = os.path.join(html_files_hostname_root, '/'.join(url_path_componets), html_file_name)
